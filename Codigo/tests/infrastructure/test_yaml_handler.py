...
from hookci.infrastructure.yaml_handler import YamlConfigHandler


@pytest.fixture
def handler_with_mock_fs() -> Tuple[YamlConfigHandler, Mock]:
    """Fixture for YamlConfigHandler with a mocked IFileSystem."""
    mock_fs = Mock(spec=IFileSystem)
    handler = YamlConfigHandler(fs=mock_fs)
    return handler, mock_fs


def test_write_config_data_serializes_correctly(
    handler_with_mock_fs: Tuple[YamlConfigHandler, Mock],
) -> None:
    """
    ...
    assert yaml.safe_load(written_content) == config_data
...
def test_load_config_data_not_a_dictionary(
    handler_with_mock_fs: Tuple[YamlConfigHandler, Mock],
) -> None:
    """Verify ConfigurationParseError is raised if YAML is not a dictionary."""
    handler, mock_fs = handler_with_mock_fs
    ...
    mock_fs.read_file.return_value = yaml_content
    with pytest.raises(
        ConfigurationParseError, match="Top-level YAML content must be a dictionary"
    ):
        handler.load_config_data(config_path)
